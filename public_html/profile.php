...
if (isset($_GET['username'])) {
    ...
    if (DB::query('SELECT username FROM usertable WHERE username=:username', array(':username'=>$_GET['username']))) {
        ...
        $followernum = DB::query('SELECT followercount FROM usertable WHERE username=:username', array(':username'=>$_GET['username']))[0]['followercount'];
                ...
                if ($user_id != $follower_id) { 
                    if ($follower_id) {
                        if (!DB::query('SELECT follower_id FROM followers WHERE user_id=:user_id AND follower_id=:follower_id', array(':user_id'=>$user_id, ':follower_id'=>$follower_id))) {
                            ...
                            DB::query('UPDATE usertable SET followercount=followercount+1 WHERE id=:user_id', array(':user_id'=>$user_id));
                        } else {
                        ...
                        echo 'Not logged in';
                        echo '<br><center></center><form method="GET" action="/login.php?">
...
<body>
    ...
    if (Login::isLoggedIn()) {
        echo '<center>'.$followernum.'</center>';
    }
